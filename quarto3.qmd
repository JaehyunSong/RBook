...
[^mirroring-off]: つまり、2台のモニターに異なる画面が映っている状態にすることを意味する。macOS（15.0.1）であれば、システム環境設定 > ディスプレイ > ディスプレイをミラーリングの**チェックを解除**すれば良い。Windows 11であれば、「<i class="bi bi-windows"></i> + P」を押せば小さいウィンドウが表示されるが、ここで「拡張」を選べば良い。ちなみに「複製」はミラーリングを意味する。
