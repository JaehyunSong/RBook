...
```{r rmarkdown-common, include = FALSE}
source("_common.R")
```
...
```{r rmarkdown-sample-1, out.width = "80%", fig.cap = "R Markdownのサンプルページ", echo = FALSE}
knitr::include_graphics("figures/Markdown/SamplePage.png")
```
...
```{r, cache = TRUE, dependson = "define_X"}`r ''`
mean(X)
```
````
...
　
````md
```{r, fig.height = 5, fig.width = 7, fig.align = "center", fig.cap = "`iris`データセットの可視化", dpi = 72}`r ''`
iris %>%
  mutate(Species2 = recode(Species,
                           "setosa"     = "セトナ",
                           "versicolor" = "バーシクル",
...
```{r rmarkdown-chunk-1, fig.height = 5, fig.width = 7, fig.align = "center", fig.cap = "`iris`データセットの可視化", dpi = 72}
iris %>%
  mutate(Species2 = recode(Species,
                           "setosa"     = "セトナ",
                           "versicolor" = "バーシクル",
...
```{r rmarkdown-chunk-2, eval = FALSE}
for(i in 1:10){
print(i*2)
}
```
...
```{r rmarkdown-chunk-3, eval = FALSE, tidy = TRUE}
for(i in 1:10){
print(i*2)
}
```
...
　このようにチャンクオプションが見やすくなるメリットがありますが、現在 (`r format(Sys.Date(), "%Y年%m月%d日")`)のRStudioは、チャンク内部のオプション指定の入力補助に対応していないことに注意してください。通常の書き方だと`if()`文のような条件分岐がチャンクのオプションとして使えますが、`#|`の書き方だと正しく作動しません。ただし、`#| eval: ...`を`#| eval = ...`にすると作動します。
