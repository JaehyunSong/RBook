...
```{r io-common, include = FALSE}
source("_common.R")
```

この章で使うパッケージを読み込む。

```{r io-setup, message = FALSE, warning = FALSE}
...
```{r io-export-4, out.width = "40%", fig.cap = "保存したcsvファイルの中身", echo = FALSE}
knitr::include_graphics("Figures/Rbasic/Export.png")
```
...
```{r io-export-10, echo = FALSE, results = "hide"}
