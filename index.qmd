...
　本書のデータは付録の「[データセット](dataset.qmd)」から入手可能である。<!--もう一つの方法は、筆者（宋）の GitHub リポジトリからダウンロードすることだ。データは以下の手順でダウンロードできる。
...
* 第3部: データハンドリング
   ...
   * 第[-@sec-datahandling2]章: データハンドリング [要約]
   * 第[-@sec-datahandling3]章: データハンドリング [拡張]
