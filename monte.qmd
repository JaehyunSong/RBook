...
tibble(
    ...
    ttest = c(round3(-diff(ttest_result$estimate)), 
              round3(ttest_result$stderr),
              paste0("[", round3(ttest_result$conf.int[1]), ", ",
                     round3(ttest_result$conf.int[2]), "]"),
              "", "", ""),
    boot  = c(round3(mean(Result_vec)), 
              round3(sd(Result_vec)),
              "",
              paste0("[", paste0(round3(quantile(Result_vec, c(0.025, 0.975))), collapse = ", "), "]"),
              paste0("[", paste0(round3(delta + qnorm(c(0.025, 0.975)) * boot_se), collapse = ", "), "]"),
              paste0("[", paste0(round3(delta - boot_b + qnorm(c(0.025, 0.975)) * boot_se), collapse = ", "), "]"))
) |> 
    gt() |>
    cols_label("Stat1" = "", "ttest" = "t検定", "boot" = "ブートストラップ") |>
