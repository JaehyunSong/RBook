...
* <https://jthomasmock.github.io/gtExtras/articles/plotting-with-gtExtras.html#graphs-vs-tables>
* `add_text_img()`
* `gt_img_rows()`
...
　一方、`read_csv()`でなく、`read.csv()`で読み込んだ表形式データのクラスはdata.frameだ。`df2`を`as.data.frame()`関数を使ってdata.frameクラスに変更してみよう。クラスを変更した`df2`は`df4_df`と名付ける。

```{r}
#| label: table-df-print-3
df4_df <- as.data.frame(df2)

class(df4_df)
```

　それでは`df4_df`を出力してみよう。
