...
　Linux MintはUbuntuをベースとしたディストリビューションであり、Ubuntu並みに普及しているメジャーなLinuxディストリビューションである。Ubuntuをベースとしている点で、以上のインストール方法はほぼ通用する。しかし、`$(lsb_release -cs)`の箇所は修正が必要である。自分が使用しているLinux MintがUbuntu 22をベースとしている場合は`$(lsb_release -cs)`の代わりに`jammy`を、Ubuntu 20がベースなら`focal`、Ubuntu 18がベースなら`bionic`など、ベースとなるUbuntuのコードネームを指定する必要がある。自信がない場合、ネットから最新情報を確認しよう。
