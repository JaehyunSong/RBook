...
　Linux MintはUbuntuをベースとしたディストリビューションであり、Ubuntu並みに普及しているメジャーなLinuxディストリビューションである。Ubuntuをベースとしている点で、以上のインストール方法はほぼ通用する。しかし、`$(lsb_release -cs)`の箇所は修正が必要である。自分が使用しているLinux MintがUbuntu 22をベースとしている場合は`$(lsb_release -cs)`の代わりに`jammy`を、Ubuntu 20がベースなら`focal`、Ubuntu 18がベースなら`bionic`など、ベースとなるUbuntuのコードネームを指定する必要がある。ちなみに2024年4月30日現在、Linux Mintの最新バージョンはLinux Mint 21.3であり、これはUbuntu 22LTSをベースとしている。自信がない場合、ネットから最新情報を確認しよう。
