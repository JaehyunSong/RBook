...
　実際の平均値の差分は`r mean(Data1) - mean(Data2)`、ブートストラップ推定量は `r mean(Result_vec)`であるため、その差は`r mean(Result_vec) - (mean(Data1) - mean(Data2))`です。これをブートストラップ推定量のバイアスと呼びます。
...
#| include: FALSE
pacman::p_load(gt)
tibble(Stat1 = c("(1): 平均値の差分", 
                 "(2): 標準誤差", 
                 "(3): 検定統計量", 
                 "(4): p値", 
                 "(5): 95%信頼区間の下限", 
                 "(6): 95%信頼区間の上限"),
       ttest = c(-diff(ttest_result$estimate), 
                 ttest_result$stderr,
                 ttest_result$statistic, 
                 ttest_result$p.value,
                 ttest_result$conf.int[1], 
                 ttest_result$conf.int[2]),
       Stat2 = c("(1): 平均値の差分の平均値", 
                 "(2): 平均値の差分の標準偏差", 
                 "(3): (1) / (2)", 
                 "(4): p値",
                 "(5): 95%信頼区間の下限", 
                 "(6): 95%信頼区間の上限"),
       boot  = c(mean(Result_vec), 
                 sd(Result_vec), 
                 mean(Result_vec) / sd(Result_vec),
                 (1 - pt(abs(mean(Result_vec) / sd(Result_vec)), df = 198)) * 2,
                 quantile(Result_vec, 0.025),
                 quantile(Result_vec, 0.975))
       ) %>%
    gt() %>%
    fmt_number(columns = c(2, 4), decimals = 3) %>%
    cols_label("Stat1" = "統計量", "ttest" = "値",
               "Stat2" = "統計量", "boot" = "値") %>%
    tab_spanner(label = md("**t検定**"), columns = 1:2) %>%
    tab_spanner(label = md("**ブートストラップ法**"), columns = 3:4)
```
