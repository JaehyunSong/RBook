...
この時点で勝手にロケール順になります。実際、表示された`Score_df`を見ると`Pref`の下に`<chr>`と表記されており、`Pref`はcharacter型であることが分かります。これをこのまま棒グラフに出してみましょう。可視化の方法は第\@ref(visualization1)章以降で詳細に解説するので、ここでは結果だけに注目してください。
...
tibble(Order = c("n = -2", "n = -1", "n = 0",
       ...
       L5    = c('"C"', '"D"', '"E"', '"F"', '"A"'),
       L6    = c('"D"', '"E"', '"F"', '"A"', '"B"')) %>%
    kable(col.names = c("水準の順番", paste0(1:6, "番目")), 
          booktabs  = TRUE, 
          align     = "lcccccc") %>%
