...
```{r iteration-common, include = FALSE}
source("_common.R")
```
...
```{r iteration-intro-4, eval = FALSE}
...
```{r iteration-intro-14, error = TRUE}
...
```{r iteration-intro-21, out.width="100%", fig.cap = "(ref:map-caption1)", echo = FALSE}
...
```{r iteration-two-args-2, eval = FALSE}
...
```{r iteration-model1-2, eval = FALSE}
# split()の使い方
...
```{r iteration-model1-6, eval = FALSE}
# 相関分析: 方法1
...
```{r iteration-split-2, eval = FALSE}
Nested_Data <- Country_df %>% 
...
```{r iteration-split-5, out.width="100%", fig.cap = "入れ子型データの生成過程", echo = FALSE}
...
```{r iteration-split-7, row.print = 25}
...
```{r iteration-split-18, eval = FALSE}
# {purrr}使用
Nested_Data %>%
  mutate(Model = map(data, ~lm(PPP_per_capita ~ FH_Total + Population, 
                               data = .x)),
...
```{r iteration-split-19, eval = FALSE}
# {purrr}を使用しない場合
...
```{r iteration-range-17, eval = FALSE}
...
```{r iteration-vars-10, eval = FALSE}
# 方法1
Diff_df <- tibble(Group = c("G7", "G20", "OECD"),
                  Formula = c(PPP ~ G7, 
                              PPP ~ G20, 
