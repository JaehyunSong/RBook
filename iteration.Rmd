...
```{r iteration-intro-21}
#| echo: FALSE
#| out.width: "100%"
#| fig.cap: "(ref:map-caption1)"
#| cache: TRUE
knitr::include_graphics("figures/Iteration/map_inside.png")
```
...
```{r iteration-model1-10}
Cor_fit2 <- Split_Data %>%
  map(~cor.test(~ Polity_Score + FH_Total, data = .x))
```

```{r iteration-model1-11}
# Cor_fit2から相関係数のみ出力
...
#| echo: FALSE
#| out.width: "100%"
#| fig.cap: "入れ子型データの生成過程"
#| cache: TRUE
knitr::include_graphics("figures/Iteration/nested_data.png")
```
...
ここからいくつかの情報が読み取れます。まず、OECD加盟国のPPPは4957943、非加盟国は211287です。その差分は-4746656です。また、*t*値は-3.4136、*p*値は0.003です。これらの情報を効率よく抽出するには`broom::tidy()`が便利です。
...
character型ですね。続いて、`map()`関数を使用して`Formula`列をformula型に変換します。
...
もう一つの方法としては最初からfromula型の変数を入れても良いでしょう。推定するモデルが多くない場合は、こちらの方が簡単かも知れません。たとえば、これまでの作業を直接formula型を格納する形式で行うなら以下のようになります[^enframe]。

[^enframe]: `enframe()`関数が登場しましたが、これはベクトル型、またはリスト型オブジェクトをtibble型に変換する関数です。
