...
```{r oop-intro-5, fig.cap = "Programmers' Hardest Tasks", out.width = "50%", echo = FALSE}
knitr::include_graphics("figures/OOP/naming.jpg")
```
...
```{r oop-about-oop-8, message = FALSE}
...
```{r oop-sample-5, out.width="100%", fig.cap = "クラスの構造", echo = FALSE}
knitr::include_graphics("figures/OOP/My_Cor_Object.png")
```

それでは一つずつ作っていきましょう。まずは、`"My_Cor_Object"`クラスのオブジェクトを作成する`My_Cor()`関数からです。

```{r oop-sample-6, eval = FALSE}
My_Cor <- function(x, y, x_name, y_name) {
    if (!is.numeric(x) | !is.numeric(y)) {
        stop("xまたはyがnumeric型ではありません。")
    }
