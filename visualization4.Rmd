...
`geom_text()`: マッピングされた`x`と`y`の箇所に文字列を付ける
...
ただし、`round()`を使う場合、`round(1.1298, 3)`は`r round(1.1298, 3)`と出力される。もし、1.130のように出力したい場合は`round()`の代わりに`sprintf()`を使用する。今回の例の場合、`sprintf("%.3f", 1.1298)`のように書く。
...
`geom_label()`: マッピングされた`x`と`y`の箇所にラベルを付ける
