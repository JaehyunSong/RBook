...
　ただし、棒と文字が重なり、やや読みにくいですね。しかも小数点も3桁くらいで十分でしょう。文字の位置をやや高めにするために`y = HDI`を`y = HDI + 0.03`に調整します。また、小数点を丸めるために`label = round(HDI, 3)`にへんこうします。ただし、`round()`を使う場合、`round(1.1298, 3)`は`r round(1.1298, 3)`と出力されます。もし、1.130のように出力したい場合は`round()`の代わりに、第\@ref(programming)章でも紹介しました`sprintf()`を使用します。今回の例の場合、`sprintf("%.3f", 1.1298)`のように書きます。
