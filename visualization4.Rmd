...
```{r visual4-common, include = FALSE}
source("_common.R")
```
...
```{r visual4-violin1, echo = FALSE, cache = TRUE, message = FALSE}
Country_df %>%
  ggplot() +
...
```{r visual4-violin2, echo = FALSE, cache = TRUE}
Country_df %>%
  ggplot() +
...
```{r visual4-violin3, echo = FALSE, cache = TRUE}
Country_df %>%
  ggplot() +
  geom_density(aes(x = log(PPP_per_capita), y = ..density..), 
               fill = "orange") +
...
```{r visual4-violin4, echo = FALSE, cache = TRUE}
Country_df %>%
  ggplot() +
  geom_density(aes(x = log(PPP_per_capita), y = ..density..), 
               fill = "orange") +
...
```{r visual4-ridge3, message = FALSE}
...
```{r visual4-pointrage1, echo = FALSE, cache = TRUE}
...
```{r visual4-pointrage2, cache = TRUE}
Pointrange_df
```
...
```{r visual4-pointrage3, cache = TRUE}
Pointrange_df %>%
    ggplot() +
...
```{r visual4-pointrage4, echo = FALSE, cache = TRUE}
...
```{r visual4-pointrage5, cache = TRUE}
Pointrange_df2
```
...
```{r visual4-pointrage6, cache = TRUE}
Pointrange_df2 %>%
    ggplot() +
    geom_hline(yintercept = 0, linetype = 2) +
    geom_pointrange(aes(x = Continent, y = Coef, 
...
```{r visual4-pointrage7, cache = TRUE}
Pointrange_df2 %>%
    ggplot() +
    geom_hline(yintercept = 0, linetype = 2) +
    geom_pointrange(aes(x = Continent, y = Coef, 
...
```{r visual4-pointrage8, cache = TRUE}
Pointrange_df2 %>%
...
```{r visual4-lollipop1, fig.height = 7, cache = TRUE}
Country_df %>%
...
```{r visual4-lollipop2, fig.height = 7, cache = TRUE}
Country_df %>%
  filter(Continent == "Europe") %>%
  drop_na(PPP_per_capita) %>%
  ggplot() +
...
```{r visual4-lollipop3, fig.height = 7, cache = TRUE}
Country_df %>%
  filter(Continent == "Europe") %>%
  drop_na(PPP_per_capita) %>%
...
```{r visual4-lollipop4, fig.height = 7.1, cache = TRUE}
Country_df %>%
  filter(Continent == "Europe") %>%
  drop_na(PPP_per_capita) %>%
  mutate(Country = fct_reorder(Country, PPP_per_capita),
...
```{r visual4-lollipop5, fig.height = 7, cache = TRUE}
Country_df %>%
  filter(Continent == "Europe") %>%
  drop_na(PPP_per_capita) %>%
  mutate(Country = fct_reorder(Country, PPP_per_capita),
...
```{r visual4-smooth1, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual4-smooth2, fig.height = 4, cache = TRUE}
Country_df %>%
    ggplot(aes(x = FH_Total, y = PPP_per_capita)) +
    geom_point() +
...
```{r visual4-smooth3, fig.height = 4, cache = TRUE}
Country_df %>%
    ggplot(aes(x = FH_Total, y = PPP_per_capita)) +
    geom_point() +
...
```{r visual4-smooth4, fig.height = 4, cache = TRUE}
Country_df %>%
    ggplot(aes(x = FH_Total, y = PPP_per_capita)) +
    geom_point() +
...
```{r visual4-smooth5, fig.height = 4, cache = TRUE}
Country_df %>%
    ggplot(aes(x = FH_Total, y = PPP_per_capita)) +
    geom_point() +
...
```{r visual4-smooth6, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual4-smooth7, fig.height = 4, cache = TRUE}
Country_df %>%
    mutate(OECD = if_else(OECD == 1, "加盟国", "非加盟国")) %>%
    ggplot(aes(x = FH_Total, y = PPP_per_capita)) +
...
```{r visual4-contour6, fig.height = 8, fig.cap = "{RColorBrewer}が提供するパレート一覧"}
...
```{r visual4-dag1, message = FALSE, echo= FALSE, cache = TRUE}
...
```{r visual4-bump4, echo = FALSE}
DT::datatable(Bump_df)
```
...
```{r visual4-alluvial3, fig.height = 7}
Vote_0910 <- Vote_0910 %>%
...
```{r visual4-alluvial4, warning = FALSE}
Vote_0910 <- Vote_0910 %>%
...
```{r visual4-alluvial5, fig.height = 7, warning = FALSE}
Vote_0910 %>%
...
```{r visual4-alluvial6, fig.height = 7, warning = FALSE}
Vote_0910 %>%
    ggplot(aes(y = Freq, axis1 = Vote09, axis2 = Vote10)) +
    geom_alluvium() +
    geom_stratum() +
...
```{r visual4-alluvial7, fig.height = 7, warning = FALSE}
Vote_0910 %>%
    ggplot(aes(y = Freq, axis1 = Vote09, axis2 = Vote10)) +
...
```{r visual4-alluvial8, fig.height = 7, warning = FALSE}
...
```{r visual4-alluvial9, fig.height = 7, warning = FALSE}
Alluvial_Plot +
...
```{r visual4-mosaic1, message = FALSE}
pacman::p_load(ggmosaic)
```

```{r visual4-mosaic2, cache = TRUE}
...
```{r visual4-mosaic5, cache = TRUE, echo = FALSE}
par(family = "HiraKakuProN-W3")
```

```{r visual4-mosaic6, cache = TRUE, fig.width = 5, fig.height = 5}
plot(Mosaic_Tab)
```
...
```{r visual4-mosaic7, cache = TRUE, fig.width = 5, fig.height = 5}
Mosaic_df %>%
    ggplot() +
...
```{r visual4-mosaic8, cache = TRUE, fig.width = 5, fig.height = 5}
Mosaic_df %>%
    ggplot() +
...
```{r visual4-mosaic9, cache = TRUE, fig.width = 8, fig.height = 5}
Mosaic_df %>%
    ggplot() +
    geom_mosaic(aes(x = product(Polity, Continent), fill = Polity), 
                na.rm = TRUE) +
...
```{r visual4-mosaic10, cache = TRUE, fig.width = 8, fig.height = 5}
Mosaic_df %>%
    ggplot() +
    geom_mosaic(aes(x = product(Polity, Continent), fill = Polity), 
                na.rm = TRUE) +
...
```{r visual4-mosaic11, cache = TRUE, fig.width = 9, fig.height = 5}
Mosaic_df %>%
    ggplot() +
    geom_mosaic(aes(x = product(Polity, Continent), fill = Polity), 
                na.rm = TRUE) +
