...
```{r tidydata-common, include = FALSE}
source("_common.R")
```
...
```{r tidydata-intro-1, echo = FALSE, message = FALSE, result = "asis"}
tibble(Name      = c("Hadley", "Song", "Yanai"),
       Control   = c(90, 80, 100),
       Treatment = c(90, 25, 95)) %>%
  kable(caption = "Messy Dataの例 (1)",
        booktabs = TRUE) %>%
...
```{r tidydata-intro-2, echo = FALSE, message = FALSE, result = "asis"}
tibble(Treat     = c("Control", "Treatment"),
       Hadely    = c(90, 90),
       Song      = c(80, 25),
       Yanai     = c(100, 95)) %>%
    kable(caption = "Messy Dataの例 (2)", booktabs = TRUE) %>%
    kable_styling(bootstrap_options = "striped",
                  full_width        = FALSE,
                  latex_options     = "hold_position") %>%
    row_spec(0, align = "c")
...
```{r tidydata-intro-4, echo = FALSE, message = FALSE}
tibble(Name       = c("Treat", "Math_Score"),
    ...
    kable(col.names = c("Name", "Hadley", "Hadley",
          ...
          caption = "表\\@ref(tab:tidydata-intro-3)を転置した場合") %>%
    kable_styling(bootstrap_options = "striped",
                  full_width        = FALSE,
                  latex_options     = "hold_position") %>%
    row_spec(0, align = "c")
...
```{r tidydata-intro-5, out.width="100%", fig.cap = "1つの列は、1つの変数を表す", echo = FALSE}
knitr::include_graphics("Figures/Tidydata/TidyData1.png")
```
...
```{r tidydata-intro-6, out.width="100%", fig.cap = "1つの行は、1つの観測を表す", echo = FALSE}
knitr::include_graphics("Figures/Tidydata/TidyData2.png")
```
...
```{r tidydata-intro-8, out.width="50%", fig.cap = "1つのセルは、1つの値を表す", echo = FALSE}
knitr::include_graphics("Figures/Tidydata/TidyData3.png")
```

図\@ref(fig:tidydata-intro-8)の例は「1セル1値」の条件に明らかに反します。しかし、基準が曖昧な変数もあり、その一つが日付です。

```{r tidydata-intro-9, echo = FALSE}
df1 <- tibble(
...
```{r tidydata-intro-10, out.width="100%", fig.cap = "国勢調査データ", echo = FALSE}
knitr::include_graphics("Figures/Tidydata/MessyData4.png")
```
...
```{r tidydata-intro-11, out.width="65%", fig.cap = "1つの表は、1つの観測単位をもつ", echo = FALSE}
knitr::include_graphics("Figures/Tidydata/TidyData4.png")
```
...
```{r tidydata-longer-1, eval = FALSE}
...
```{r tidydata-longer-3, echo = FALSE}
df1 %>%
...
「1列1変数」を満たさなかった最初のデータは「Wide型データ」、これを満たすようなデータは「Long型データ」と呼ばれます。これは**相対的**に最初のデータが横に広いから名付けた名前であって、「Wide型=雑然データ」もしくは「Long型=整然データ」ではないことに注意してください。実際、表\@ref(tab:tidydata-intro-7)はどれも同じ情報を持つ表であり、右の方がLong型データです。しかし、整然データはWide型である左の方ですね。つまり、「Long型データだから1列1変数だ」ということは間違いです。同じ情報を持つデータで、相対的にどれが横長か、縦長かの相対的な問題です。

Wide型データをLong型へ変換する関数が`pivot_longer()`であり、基本的な使い方は以下の通りです。

```{r tidydata-longer-4, eval = FALSE}
# pivot_longer()の使い方
データ名 %>%
...
```{r tidydata-wider-1, echo = FALSE, message = FALSE}
...
```{r tidydata-wider-3, echo = FALSE, message = FALSE}
...
```{r tidydata-separate-1, message = FALSE}
...
```{r tidydata-separate-2, echo = FALSE, message = FALSE}
...
```{r tidydata-separate-4, eval = FALSE}
データ名 %>%
...
```{r tidydata-separate-9, eval = FALSE}
# separate()の使い方
データ名 %>%
