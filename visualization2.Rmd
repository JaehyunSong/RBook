...
```{r visual2-sampledata-1, echo = FALSE, message = FALSE, cache = TRUE}
data.frame(変数名 = paste0("`",names(Country_df),"`"),
              説明 = c("国名", "人口", "面積", 
                     "国内総生産 (GDP)", "GDP (購買力平価):",
...
```{r visual2-sampledata-2, echo = FALSE, message = FALSE, cache = TRUE}
data.frame(変数名 = paste0("`",names(COVID19_df),"`"),
              説明 = c("ID", "国名", "年月日",
                     "COVID-19 新規感染者数（人）", 
                     "COVID-19 累積感染者数（人）",
                     "COVID-19 新規死亡者数（人）", 
...
```{r visual2-histogram-1, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
...
```{r visual2-histogram-2, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
  geom_histogram(aes(x = GDP)) +
...
```{r visual2-histogram-3, echo = FALSE, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
  geom_histogram(aes(x = GDP)) +
...
```{r visual2-histogram-4, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  filter(GDP < 1000000) %>%
  ggplot() +
  geom_histogram(aes(x = GDP)) +
...
```{r visual2-histogram-5, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
  geom_histogram(aes(x = GDP)) +
  labs(x = "国内総生産 (兆米ドル)", y = "度数") +
...
```{r visual2-histogram-6, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
  geom_histogram(aes(x = GDP)) +
  labs(x = "国内総生産 (兆米ドル)", y = "度数") +
...
```{r visual2-histogram-7, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>% 
  ggplot() +
  geom_histogram(aes(x = GDP)) +
  labs(x = "国内総生産 (兆米ドル)", y = "度数") +
...
```{r visual2-histogram-8, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  ggplot() +
...
```{r visual2-histogram-9, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual2-histogram-10, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-histogram-11, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-histogram-12, echo = FALSE, message = FALSE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ...
  knitr::kable(col.names = c("から", "まで", "度数")) %>%
...
```{r visual2-histogram-13, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
  geom_histogram(aes(x = HDI_2018), color = "white", 
...
```{r visual2-histogram-14, echo = FALSE, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
...
```{r visual2-histogram-15, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
...
```{r visual2-histogram-16, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot(aes(x = HDI_2018, y = ..density..)) +
  geom_histogram(color = "white", binwidth = 0.05, boundary = 0) +
...
```{r visual2-histogram-17, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual2-histogram-18, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual2-histogram-19, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  mutate(OECD2 = ifelse(OECD == 1, "OECD加盟国", "OECD非加盟国")) %>%
  filter(!is.na(HDI_2018)) %>%
...
```{r visual2-histogram-20, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  mutate(OECD2 = ifelse(OECD == 1, "OECD加盟国", "OECD非加盟国")) %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-histogram-21, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
  geom_histogram(aes(x = HDI_2018), 
...
```{r visual2-summary-1, out.width="85%", fig.cap = "ggplot2 aesthetics cheatsheet", echo = FALSE}
