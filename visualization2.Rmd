...
```{r visual2-common, include = FALSE}
source("_common.R")
```
...
　その他の図や、図の細かい修正については第\@ref(visualization3)章で解説します。また、使用するPCの環境によっては図の文字化けが生じる可能性があります。この場合は第\@ref(visual2-japanese)章を参照してください。
...
#| message: FALSE
#| cache: TRUE
data.frame(Var  = names(Country_df),
           Name = c("国名", "人口", "面積", 
                    "国内総生産 (GDP)", "GDP (購買力平価):",
           ...
           Desc = c("", "人", "km$^2$", 
                    ...
                    "")) %>%
    kable(caption   = "国家別民主主義および政治的自由データの詳細",
          col.names = c("変数名", "説明", "備考"), 
          booktabs  = TRUE, 
          align     = "lll") %>%
...
#| message: FALSE
#| cache: TRUE
data.frame(Var = names(COVID19_df),
           Desc = c("ID", "国名", "年月日",
                    "COVID-19 新規感染者数（人）", 
                    "COVID-19 累積感染者数（人）",
                    "COVID-19 新規死亡者数（人）", 
...
```{r visual2-barplot-2, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  ggplot() +
...
```{r visual2-barplot-4, fig.width = 8, fig.height = 4, cache = TRUE}
Bar_df1 %>%
  ggplot() +
...
```{r visual2-barplot-5, fig.width = 8, fig.height = 4, cache = TRUE}
Bar_df1 %>%
...
```{r visual2-box-1, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-box-2, echo = FALSE, fig.width = 8, fig.height = 6, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
  geom_boxplot(aes(y = HDI_2018)) +
...
```{r visual2-box-3, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
  geom_boxplot(aes(y = HDI_2018)) +
...
```{r visual2-box-4, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-box-5, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot() +
...
```{r visual2-box-6, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
...
```{r visual2-box-7, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  filter(!is.na(HDI_2018)) %>%
  ggplot(aes(x = Continent, y = HDI_2018)) +
...
```{r visual2-box-8, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
...
```{r visual2-box-9, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  mutate(Continent2 = factor(Continent,
                             levels = c("Africa", "America", "Asia",
                                        "Europe", "Oceania"),
...
```{r visual2-box-10, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  mutate(Continent2 = factor(Continent,
                             levels = c("Africa", "America", "Asia",
                                        "Europe", "Oceania"),
...
```{r visual2-box-12, fig.width = 8, fig.height = 4, cache = TRUE}
Democracy_df %>%
  ggplot() +
...
```{r visual2-box-13, fig.width = 8, fig.height = 4, cache = TRUE}
Democracy_df <- Country_df %>% 
  select(Country, Continent, Polity_Score, FH_Total) %>%
  # FH_Totalのすケース調整
...
```{r visual2-scatter-1, fig.width = 8, fig.height = 4, cache = TRUE}
Country_df %>%
  ggplot() +
...
```{r visual2-scatter-2, cache = TRUE}
Country_df %>% 
...
```{r visual2-scatter-3, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
  ggplot() +
  geom_point(aes(x = PPP_per_capita, y = HDI_2018)) +
  labs(x = "一人当たり購買力平価GDP (USD)", y = "人間開発指数") +
...
```{r visual2-scatter-4, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
...
```{r visual2-scatter-5, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
  mutate(Area2 = Area / 1000000) %>%
  ggplot() +
  geom_point(aes(x = PPP_per_capita, y = HDI_2018, 
...
```{r visual2-scatter-6, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
  mutate(Area2 = Area / 1000000,
...
```{r visual2-scatter-7, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
  mutate(Area2    = Area / 1000000,
...
```{r visual2-scatter-9, eval = FALSE}
# gghighlight()の使い方
...
```{r visual2-scatter-10, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
Country_df %>%
  mutate(Area2 = Area / 1000000) %>%
  ggplot() +
  geom_point(aes(x = PPP_per_capita, y = HDI_2018, 
...
```{r visual2-line-2, fig.width = 8, fig.height = 4, cache = TRUE}
COVID19_df %>%
  ggplot() +
...
```{r visual2-line-3, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
COVID19_df %>%
  ggplot() +
...
```{r visual2-line-4, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
COVID19_df %>%
  ggplot() +
...
```{r visual2-line-5, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
...
```{r visual2-line-6, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE}
COVID19_df %>%
  filter(Country %in% G7) %>%
...
```{r visual2-line-7, fig.width = 8, fig.height = 4, cache = TRUE, warning = FALSE, message = FALSE}
COVID19_df %>%
  ggplot() +
  geom_line(aes(x = Date, y = Confirmed_Total, color = Country)) +
...
```{r visual2-save-ggsave-1, eval = FALSE}
# ggsave()を利用した保存方法
...
```{r visual2-save-quartz-1, eval = FALSE}
# quartz()を利用した保存方法
...
```{r visual2-save-quartz-4, eval = FALSE}
# 方法1
...
```{r visual2-save-ragg-1, eval = FALSE}
...
```{r visual2-save-ragg-2, eval = FALSE}
...
#| echo: FALSE
#| out.width: "85%"
#| fig.cap: "ggplot2 aesthetics cheatsheet"
knitr::include_graphics("figures/Visualization2/ggplot_aesthetics_cheatsheet.png")
```
