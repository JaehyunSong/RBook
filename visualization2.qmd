...
　その他の図ついては第[-@sec-visualization4]章、図の細かい修正については第[-@sec-visualization3]章で解説します。また、使用するPCの環境によっては図の文字化けが生じる可能性があります。この場合は第[-@sec-visual2-japanese]、[-@sec-visual2-save]章を参照してください。
...
　これは「連続変数`HDI_2018`を30区間（=階級）に分けました」という意味です。この区間数を調整する方法は2つあり、区間数を指定する（1）、区間の幅を指定する（2）方法があります。
...
　図に日本語を表示するだけなら、macOSと方法は同じですが、使用するフォントを`"HiraginoSans-W3"`から`"Yu Gothic"`に変更します。

### {ragg}の使用

　{ragg}パッケージを使用すると、使用するOSと関係なくRStudioのPlotsペイン、Quarto、R Markdown（HTML出力）の文字化けを回避することができます。{ragg}はインストールのみで十分であり、別途読み込む必要がないため、インストールだけします。
...
　図の保存方法は色々ありますが、ここでは{ggplot2}パッケージが提供する`ggsave()`関数を使用した例を紹介します。
...
　もし、{ragg}が導入済みである場合、`device`引数に`"png"`の代わりに、`ragg::agg_png`を割り当てれば文字化けの心配なく、PNG形式で図の保存ができます（日本語 + 韓国語 + 中国語もOK）。

### PDF形式で保存

　PDF形式でも`ggsave()`の使い方はほぼ変わりません。`device`を`"png"`や`ragg::agg_png`から`cairo_pdf`、または`CairoPDF`にするだけです（`CairoPDF`を使う場合は{Cairo}パッケージが必要）。ここでは{Cairo}パッケージが提供するCairoPDFデバイスを使用した例を紹介します。

```{r visual2-save-cairo-1}
#| eval: false
# {Cairo}のCairoPDFを使用する例
pacman::p_load(Cairo)
...
BarPlot1 <- Country_df |>
  ...
  geom_bar(aes(x = Continent)) +
  labs(x = "大陸", y = "ケース数") +
  theme_bw(base_family = "HiraginoSans-W3") # <1>

ggsave(filename = "Figs/BarPlot.pdf",       # <2>
       plot     = BarPlot1,
       device   = CairoPDF,                 # <3>
       width    = 5,
       height   = 5,
...
1. 使用するフォントを指定する。ここで指定したフォントがPDF内に埋め込まれる。Windowsなら "Yu Gothic" でOK
2. 拡張子はpngでなく、pdfを使用
3. `"png"`の代わりに`CairoPDF`を使用する。ただし、予め{Cairo}パッケージを読み込んでおくか、{Cairo}はインストールだけしておき、`Cairo::CairoPDF`を使用する。

　一つ注意すべき点としては予め`theme_update()`などでフォントを指定しておかなかった場合は`theme_*()`、または`theme()`で当該プロットが使用するフォントをしてしておく必要があることです。

　他にもよく使うフォントとしてNotoシリーズがありますが、たとえばNoto Sans Japanse（Noto Sans JP）を使う場合は`"HiraginoSans-W3"`、または`"Yu Gothic"`を`"Noto Sans JP"`に変えるだけです（ただし、予め[Google Fonts](https://fonts.google.com/noto/specimen/Noto+Sans+JP)からダウンロードしておく必要があります）。
